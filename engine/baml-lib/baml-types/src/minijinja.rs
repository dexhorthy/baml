use crate::{baml_value::BamlMapKey, BamlMedia, BamlValue};
use std::fmt;
...
impl From<BamlMapKey> for minijinja::Value {
    fn from(key: BamlMapKey) -> Self {
        match key {
            BamlMapKey::String(s) => minijinja::Value::from(s),
            BamlMapKey::Int(n) => minijinja::Value::from(n),
        }
    }
}
